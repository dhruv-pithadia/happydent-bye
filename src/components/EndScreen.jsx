...
const EndScreen = ({ onRetry, onRetryAR, userData }) => {
  ...
  const downloadPhoto = async () => {
    ...
    try {
      console.log(`📥 Downloading photo from S3 URL: ${s3ImageUrl}`);

      // Fetch the image directly from S3 URL
      const response = await fetch(s3ImageUrl, {
        method: "GET",
        mode: "cors", // Enable CORS for S3 requests
      });
      ...
      // Set filename using sessionId or fallback
      const sessionId = sessionInfo?.sessionId || "session";
      const fileName = `${sessionId}_happydent_photo.jpg`;
